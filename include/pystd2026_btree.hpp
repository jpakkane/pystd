...
public:
    void insert(const Payload &value) {
        Payload p(value);
        insert(::pystd2026::move(p));
    }

    void insert(Payload &&value) {
        if(is_empty()) {
            create_root_node(::pystd2026::move(value));
...
private:
    ...
    void insert_recursive(Payload &value, NodeReference current_node_id) {
        ...
        if(insert_loc < current_node.values.size() && current_node.values[insert_loc] == value) {
            current_node.values[insert_loc] = ::pystd2026::move(value);
            return;
        }
        if(current_node_id.to_leaf) {
            current_node.values.insert(insert_loc, ::pystd2026::move(value));
    ...
    NodeReference split_internal_node(NodeReference node_id) {
        ...
        internals.push_back(::pystd2026::move(new_node)); // Invalidates to_split_before_push.
        InternalNode &to_split = get_internal(node_id);
        NodeReference right_node_id{(uint32_t)internals.size() - 1};
        if(node_id == root) {
            InternalNode new_root;
            new_root.parent = NodeReference::null_ref();
            new_root.values.push_back(::pystd2026::move(value_to_move));
            ...
            insert_nonfull(::pystd2026::move(value_to_move), to_split.parent, right_node_id);
    ...
    void insert_nonfull(Payload &&value, NodeReference node_id, NodeReference right_id) {
        ...
        if(insert_loc == node.values.size()) {
            node.values.push_back(::pystd2026::move(value));
            node.children.push_back(::pystd2026::move(right_id));
        } else {
            assert(value < node.values[insert_loc]);
            node.values.insert(insert_loc, ::pystd2026::move(value));
            node.children.insert(insert_loc + 1, ::pystd2026::move(right_id));
        }
    }
    ...
    void create_root_node(Payload &&p) {
        ...
        n.values.push_back(::pystd2026::move(p));
        leaves.push_back(::pystd2026::move(n));
    }
    ...
    extract_value_from_leaf(const Payload &value, NodeReference node_id, uint32_t node_loc) {
        ...
        if(!(prospective < value) && !(value < prospective)) {
            Optional<Payload> value = ::pystd2026::move(current_node.values[node_loc]);
    ...
    extract_value_from_internal(const Payload &value, NodeReference node_id, uint32_t node_loc) {
        ...
        if(lc.size() > MIN_VALUE_COUNT) {
            ...
            Payload return_value = ::pystd2026::move(p.values[node_loc]);
            p.values[node_loc] = ::pystd2026::move(predecessor.values.back());
            ...
            Payload return_value = ::pystd2026::move(p.values[node_loc]);
            p.values[node_loc] = ::pystd2026::move(successor.values.front());
            ...
            assert(sub_value);
            return ::pystd2026::move(sub_value.value());
        }
        abort();
    }
